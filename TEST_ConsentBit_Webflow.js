...
(function() {
  ...
  (function () {
    ...
    async function showAppropriateBanner() {
      // Hide all banners first to avoid overlap
      hideAllBanners();
      
      // Check data-all-banners attribute
      ...
      if (hasAllBannersAttribute && allBannersValue === 'false') {
        ...
        setTimeout(async () => {
          ...
          if (detectedLocation) {
            // Update global locationData with actual detected location (but keep bannerType as GDPR)
            window.locationData = {
    ...
    async function checkPublishingStatus() {
      try {
        ...
        let apiUrl = `https://cb-server.web-8fb.workers.dev/api/site/subscription-status?siteDomain=${encodeURIComponent(siteDomain)}`;
        if (siteId) {
    ...
    document.addEventListener('DOMContentLoaded', async function () {
      // CRITICAL: First, ensure all Google scripts are unblocked (even if they have type="text/plain" or data-category)
      unblockGoogleScripts();
